        ...
        try {
            ...
            try {
                if (mContext != null && intent != null) {
                    IpcMonitorHelper.getInstance().ensureInitialized(mContext);
            
                    String sender = (callerPackage != null) ? callerPackage : "system";
            
                    // If it is explicit BroadcastReceiver, we know receiver
                    String receiver = (intent.getComponent() != null) ? intent.getComponent().getPackageName() : (intent.getPackage() != null ? intent.getPackage() : "*");

                    JSONObject payload = new org.json.JSONObject();
                    payload.put("action", intent.getAction());
            

                    IpcMonitorHelper.getInstance().report(
                        mContext, "BroadcastReceiver", sender, receiver, payload
                    );
                }
            } catch (Exception e) {
                Slog.e("IPC_MONITOR", "Failed to report BroadcastReceiver IPC", e);
            }
            // END CUSTOM IPC MONITOR
            return res;
        } finally {
