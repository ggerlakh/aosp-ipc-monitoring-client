...
public class ContentProviderHelper {
    ...
    private static final String ACTION_IPC_MONITOR = "com.custom.aosp.IPC_MONITOR_EVENT";
    private static final String MONITOR_PKG = "com.example.ipcmonitorclient";

    /**
     * Метод для отправки информации об IPC ContentProvider.
     * @param callerApp процесс, который запрашивает доступ (Client)
    ...
    private void reportCpInteraction(ProcessRecord callerApp, ContentProviderRecord cpr) {
        ...
        mService.mHandler.post(() -> {
            try {
                String sender = (callerApp != null && callerApp.info != null) 
                                ? callerApp.info.packageName : "system";
            
                // Если cpr null или app info null, значит что-то пошло не так или это системный процесс
                if (cpr == null || cpr.info == null) return;
            
                String targetPkg = cpr.info.packageName;
                String authority = cpr.info.authority;

                // 1. Фильтр рекурсии и шума
                ...
                // 3. Отправка Intent
                Intent monitorIntent = new Intent(ACTION_IPC_MONITOR);
