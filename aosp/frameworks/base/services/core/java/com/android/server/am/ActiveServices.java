...
public final class ActiveServices {
    ...
    ComponentName startServiceLocked(IApplicationThread caller, Intent service, String resolvedType,
        ...
        // START CUSTOM IPC MONITOR
        try {
            if (r != null && mAm.mContext != null) {
                final ProcessRecord callerApp = mAm.getRecordForAppLOSP(caller);

                IpcMonitorHelper.getInstance().ensureInitialized(mAm.mContext);
            ...
            Slog.e("IPC_MONITOR", "Failed to report Service IPC", e);
        }
        // END CUSTOM IPC MONITOR
        ...
        try (var unused = mAm.mProcessStateController.startServiceBatchSession(
            ...
            if (Trace.isTagEnabled(Trace.TRACE_TAG_ACTIVITY_MANAGER)) {
                ...
                if (clist.size() > 0) {
                   ...
                   // START CUSTOM IPC MONITOR
                   try {
                       if (r != null && r.binding != null && mAm.mContext != null) {

                           IpcMonitorHelper.getInstance().ensureInitialized(mAm.mContext);
                           ProcessRecord clientApp = r.binding.client;
                           String sender = clientApp.info.packageName;

                           ServiceRecord service = r.binding.service;
                           String receiver = service.packageName;
