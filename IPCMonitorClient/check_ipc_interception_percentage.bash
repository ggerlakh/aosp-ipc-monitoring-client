...
Примеры:
  ...
  $0 -v --trace-duration-secs 120         # Трассировка на 120 секунд с подробным выводом
...
then
    echo "Найден старый файл $ATRACE_OUTPUT_FILE, удаляем его для перезаписи"
    rm $ATRACE_OUTPUT_FILE
fi

adb pull /data/local/tmp/$ATRACE_OUTPUT_FILE
timeout 2s adb logcat -s IpcMonitorReceiver > ipc_monitor_data.txt
if [[ -f "$IPC_MONITOR_OUTPUT_FILE" ]]
then
    echo "Найден старый файл $IPC_MONITOR_OUTPUT_FILE, удаляем его для перезаписи"
    rm $IPC_MONITOR_OUTPUT_FILE
fi
...
do
    ...
    if [[ "$VERBOSE" == "true" ]]
    then
        echo "atrace_provider_record = $atrace_provider_record"
...
while read -r target_line && read -r intent_line; do
    ...
    intercepted_ipc_monitor_broadcast_data=$(jq -Mrc --arg type "BroadcastReceiver" --arg broadcast_target "$broadcast_target" --arg broadcast_action "$broadcast_action" 'select(.type == $type and .receiver == $broadcast_target and .payload.action == $broadcast_action)' $IPC_MONITOR_OUTPUT_FILE | head -1)
...
echo "ContentProvider: ${CONTENT_PROVIDER_IPC_INTERCEPTION_PERC}% (intercepted = ${CONTENT_PROVIDER_INTERCEPTED_IPC_COUNT}, total = ${CONTENT_PROVIDER_TOTAL_IPC_COUNT})"
echo "Service: ${SERVICE_IPC_INTERCEPTION_PERC}% (intercepted = ${SERVICE_INTERCEPTED_IPC_COUNT}, total = ${SERVICE_TOTAL_IPC_COUNT})"
echo "BroadcastReceiver: ${CONTENT_PROVIDER_IPC_INTERCEPTION_PERC}% (intercepted = ${BROADCAST_RECEIVER_INTERCEPTED_IPC_COUNT}, total = ${BROADCAST_RECEIVER_TOTAL_IPC_COUNT})"